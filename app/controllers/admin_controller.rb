class AdminController < ApplicationController
  ...
  def index
    ...
    @completed_lesson_by_day = LessonUser.where("created_at > ?", 1.week.ago).where(completed: true).group_by_day(:created_at, format: "%A").count

    @sign_ups_by_day = User.where("created_at > ?", 1.week.ago).group_by_day(:created_at, format: "%A").count

    @most_popular_courses = Course.joins(:course_users).group(:id).order("count(course_users.course_id) desc").limit(5)
  end
end
